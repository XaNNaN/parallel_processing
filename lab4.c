...
int main(int argc, char* argv[]){
	...
	FILE *fp;
	double *new, *old, *tmp, *sending_new;
	...
	while (cur_time < modeling_time){
		...
		new[0] = new[N-1] = new[(M-1) * N] = new[M*N -1] = U0;
		// вынесение значений на жёсткий диск
		// new_to_all();
		for (i = bot_row; i < top_row; i++ ){
			for (j = 0; j < N; j++){
				sending_new[(i - bot_row) * N + j] = new[N * i + j];
			}
		}
		
		
		
		double * ptr = &new[bot_row * N];
